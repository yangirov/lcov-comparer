...
function generateHtmlReport(rows, outFile) {
...
function generateJsonReport(rows, outFile) {
  fs.writeFileSync(outFile, JSON.stringify(rows, null, 2), 'utf8');
  console.log(`✅ ${outFile} saved`);
}

function generateCsvReport(rows, outFile) {
  const headers = [
    'file',
    'statements_from',
    'statements_to',
...
function run(fileA, fileB, options = {}) {
  const { outFile = 'coverage-diff.html', threshold = null } = options;
  ...
  if (outFile.endsWith('.html')) generateHtmlReport(rows, outFile);
  else if (outFile.endsWith('.json')) generateJsonReport(rows, outFile);
  else if (outFile.endsWith('.csv')) generateCsvReport(rows, outFile);
  else {
    console.error('❌ Unsupported output format. Use .html, .json or .csv');
    process.exit(1);
  }
}
